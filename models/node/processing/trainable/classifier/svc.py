...
class SVC(SKLearnClassifier):
    """ This node is a wrapper for the ``SVC`` classifier from sklearn. It is a subclass of
    ``SKLearnClassifier`` and implements the abstract methods from it. The ``SVC`` classifier
    can be used to classify data into two or more classes. 

    Attributes:
        _MODULE_NAME (str): The name of the module(in this case ``node.processing.trainable.classifier.SVC``)

    configuration.json usage: 
        **module** (*str*): The name of the module (``node.processing.trainable.classifier``)\n
        **type** (*str*): The name of the class (``SVC``)\n
    ...
    """
    _MODULE_NAME: Final[str] = 'node.processing.trainable.classifier.svc'
    ...
    def _validate_parameters(self, parameters: dict):
        ...
        #     raise MissingParameterError(module=self._MODULE_NAME, name=self.name, parameter='kernel')
        # if type(parameters['kernel']) is not str:
        #     raise InvalidParameterValue(module=self._MODULE_NAME, name=self.name, parameter='kernel', cause='must_be_str')
    ...
    def _initialize_parameter_fields(self, parameters: dict):
        ...
        # self._kernel = parameters['kernel']

    def _initialize_trainable_processor(self) -> Tuple[TransformerMixin, BaseEstimator]:
        """ Initializes the trainable processor. In this case it initializes the ``SVC`` classifier
        from sklearn.

        :return: The initialized ``SVC`` classifier.
        :rtype: (TransformerMixin, BaseEstimator)
        """
        return SVC(kernel='linear') #self._kernel)
